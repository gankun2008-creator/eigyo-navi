...
- 類似企業検索
- チーム・課題管理（タスク、送信担当、取引先権限、営業受付企業の課題DB）
- 登録企業限定の課題マッチング・企業間DM（未登録企業への送信はサーバー側で拒否）
- 3か月無料トライアル、月100チケット、早期契約時の6か月均等割引
- アカウント専用DM（本人以外の履歴は非表示）とプロフィール編集
- 登録企業2,000社分の直接登録課題データと、登録課題だけを根拠にする候補検索
- 設定
